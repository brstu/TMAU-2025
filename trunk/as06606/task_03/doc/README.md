...
## 2.
Изучил [Руководство](https://github.com/savushkin-rd/PLCnext-howto/tree/master/HowTo%20build%20program%20Hello%20PLCnext).
...
Обязательно нужно убедиться в наличии файла * toolchain.cmake * и папок * sysroots * , так как чаще всего из-за них используются ошибки.
### Сборка
...
Необходимо проверить возможность подключения к контроллеру с помощью команды * ping 192.168.1.10 * (ip контроллера). При успешном обмене файлами можно перейти к соединению с с контроллером.
...
После этого происходит авторизация. Имя пользователя - admin, пароль указан в верхней части контроллера. Мы входим в терминал, откуда можем писать необходимые команды.
