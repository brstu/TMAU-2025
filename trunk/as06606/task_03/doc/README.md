...
В программе PuTTY указывается соответствующий IP-адрес контроллера, далее — **Открыть**.
...
После этого происходит авторизация. Имя пользователя - admin, пароль указан в верхней части контроллера. Мы входим в терминал, куда можем попасть необходимые команды.
...
С ее помощью мы получаем доступ к файлам системы двух устройств и можем передавать файлы.

После того, как файл находится на контроллере, необходимо в программе PuTTY командами перейти в каталог к ​​файлу и запустить его. Команда для запуска выполнения и результат представлены ниже.
``` bash
