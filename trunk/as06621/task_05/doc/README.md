...
Для работы демо‑проекта необходимо настроить основные SCADA‑компоненты из репозитория `SCADA-ptusa-bin`.

**DbEditorXML**

- Запустите `DbEditorXML.exe` из каталога `SCADA-ptusa-bin`.
- Откройте проект конфигурации (меню **File → Open**) и укажите файл XML‑конфигурации из репозитория `T1-PLCnext-Demo` (каталог `sys` или другой, указанный в методических указаниях).
- Проверьте и при необходимости скорректируйте пути к проекту и базе данных (директории `sys`, журналы, архивы).
- Сохраните конфигурацию и закройте редактор.

**EasyServer**

- Запустите `EasyServer.exe` из каталога `SCADA-ptusa-bin`.
- В настройках сервера загрузите конфигурационный файл проекта (файл `*.ini` или `*.cfg`, поставляемый вместе с демо‑проектом).
- Убедитесь, что указаны корректные порты и IP‑адрес (как правило, `localhost` и стандартный порт, приведённый в методических указаниях).
- Запустите сервер и убедитесь, что он перешёл в состояние ожидания подключений.

**Monitor**

- Запустите `Monitor.exe` из каталога `SCADA-ptusa-bin`.
- В диалоге подключения укажите адрес сервера (обычно `localhost` и порт, настроенный в EasyServer).
- Загрузите экран(ы) визуализации или проект мониторинга, поставляемые вместе с демо‑проектом (`T1-PLCnext-Demo`).
- Убедитесь, что теги и переменные считываются корректно (значения обновляются при работе контроллера).
