...
#### Подготовка
Первым шагом скачиваем исходный код проекта с платформы GitHub с использованием команды git clone:
```
...
```
Этот файл необходимо сделать исполняемым, например с помощью команды chmod 755 ptusa_main (не рекомендуется использовать chmod 777).
...
```
### Итог
Мною был был собран и запущен проект ptusa_main на тестовом контроллере Phoenix Contact AXC F 2152.
Проект работает, но были выявлены сетевые ошибки при взаимодействии с устройством по адресу 192.168.1.11 и с EasyServer.

### Вывод
Задача выполнена, проект запускается, однако требуется дополнительная настройка сетевых соединений, которая будет выполняться в лабораторной работе №5.