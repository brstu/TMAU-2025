...
# Ход работы
## 1 - 2. 
Работа производиться с контроллером, который применялся для выполнения задания 3.
...
Соответствие адреса необходима для корректной сборки.
...
Для файла ptusa_main устанавливаем права доступа на чтение, запись и исполнение (777), чтобы не возникало вопросов с правами доступа.
Запускаем проект командой ```./ptusa_main  main.plua  sys_path ./sys/```.
...
Скачиваем ее на раюочий компьютер, и также помещаем в каталог с проектом.

После повторного введения команды на запуск
проект запустился. Получили следующее сообщение:

```
admin@axcf2152:/opt/main$ ./ptusa_main_Vanya  main.plua  sys_path ./sys/
...
```
Таким обрахом, собранный проект запущен на тестовом контроллере.
