...
убедиться в наличии файла *toolchain.cmake* и папки *sysroots*, так как чаще всего изза них происходили ошибки.
### Сборка
...
Находиться данныйй файл по адресу *C:\Users\Brest\PLCnext\Hello-PLCnext\bin\AXCF2152_2021.6.0 (21.6.0.46)*. В соответствии с заданием стандартное название изменено на ***Hello PLCnext from AS06503***. 
...
Для этого хаходим в параметры сети -> Ethernet -> настройка параметров адаптера. Там заходим в свойства Ethernet 
...
С использованием Ethernet-кабеля контроллер подкличается к компьютеру.

В командной строке вводиться команда *ipconfig* для проверки наличия подключенных устройств. Если данный порт активен, 
необходимо проверить возможность подключения к контроллеру командой *ping 192.168.1.10* (ip контроллера). При успешном обмене файлами можно переходить к соединению с контроллером.
В программе PuTTY вводиться соответствующий IP контроллера, далее - **Open**.
...
В программе WinSCP также выполняем подкючение и авторизацию. 
...
После того, как файл находиться на контроллере, необходимо в программе PuTTY командами перейти в каталог к файлу, и выполнить его. Команда для запуска исполнения и результат вывода представлены ниже.
```bash
...
**Вывод:** Контроллер AXCF2152 по своей сути является ЭВМ, к которой можно подключиться и запустить исполнение файлов. В данной работе продемонстрированы возможности по консольномы выводу текстового сообщения с использование данного контроллера.