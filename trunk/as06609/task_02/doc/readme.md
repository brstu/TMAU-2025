...
4. В файле `readme.md` отразить количество написанных тестов и процент покрытия кода тестами (использовать любой инструмент для анализа покрытия, например, OpenCppCoverage).  
...
Для проверки покрытия кода тестами использовали OpenCppCoverage. Для запуска тестов использовали скрипт:

```bash
"C:\Users\kugan\Desktop\Tools\OpenCppCoverage\OpenCppCoverage.exe" ^
--export_type=html:"C:\Users\kugan\Desktop\ТиМАУ\task_02\src\build\coverage" ^
--sources="C:\Users\kugan\Desktop\ТиМАУ\task_02\src" ^
-- "C:\Users\kugan\Desktop\ТиМАУ\task_02\src\build\Debug\runTests.exe"
```
Таким образом получили файл index.html, в котором отражены результаты тестирования. Общий процент покрытия кодом составляет ~26%, но в него также включаются и файлы используемых библиотек. Значимым показателем будем считать процент покрытия тестами только для файла func.cpp, в котором содержатся тестируемые функции. Для данного файла процент покрытия составляет 100%.

Вывод
В ходе работы были разработаны и выполнены модульные тесты для функций linear() и nonlinear() с использованием фреймворка Google Test. Все тесты успешно прошли, что подтвердило корректность реализованных алгоритмов. Анализ покрытия кода с помощью OpenCppCoverage показал общее покрытие около 26%, при этом пользовательские функции протестированы полностью.