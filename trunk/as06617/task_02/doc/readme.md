...
TEST(NonLinear, test_u_zero) {
    EXPECT_DOUBLE_EQ(nonlinear(18, 18, 0, 0), a * 18 - b * pow(18, 2));comments. Press enter to view.
}
...
TEST(NonLinear, test_default) {
    EXPECT_DOUBLE_EQ(nonlinear(18, 18, 5, 5), a * 18 - b * pow(18, 2) + c * 5 + d * std::sin(5));view.
}
...
Вывод: В ходе лабораторной работы были созданы и выполнены модульные тесты для функций linear() и nonlinear() с использованием Google Test. Все восемь тестов прошли успешно, что подтверждает корректную работу функций. По результатам анализа покрытия (например, с помощью OpenCppCoverage) пользовательский файл func.cpp имеет 100% покрытие, хотя суммарное покрытие проекта около 26% из-за включения сторонних библиотек.