...
void testLinearModel() {
    ...
    for (size_t t = 0; t < control_inputs.size(); ++t) {
        ...
        std::cout << std::fixed << std::setprecision(1) << std::setw(4) << t + 1 << std::setw(8) << u << std::setw(12) << y << std::endl;
    }
}
...
void testNonlinearModel() {
    ...
    for (size_t t = 0; t < control_inputs.size(); ++t) {
        double u = control_inputs[t];
        double y = model.calculateNext(u);
        std::cout << std::fixed << std::setprecision(3) << std::setw(4) << t + 1 << std::setw(8) << u << std::setw(12) << y << std::endl;
    }
}
