...
### Заключение
Локально проект собрался и запустился, Lua-скрипты выполняются, изменения (A3AO2, blink_fast) работают. На контроллере проект также запускается и показывает ожидаемый результат
