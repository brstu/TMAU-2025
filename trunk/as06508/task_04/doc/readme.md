...
Далее важно не забыть передать все права нашему исполняемому файлу ptusa_main(можно прожать все галочки в настройках или chmod 777)
...
2023-12-20 03.54.20 ERROR(3) -> 1-1-2 : Нет связи с узлом I/О 'A100' ('192.1681.11', 111-PICnext-Demo")
```
...
Эта ошибка повторяется много раз возможно потому,  контроллер не может установить связь с устройством A100 по адресу 192.168.1.11. Устройство не отвечает (таймаут 100 мс)
...
Но так как задача лабораторной просто:Используя Visual Studio собрать данные проект и продемонстрировать работоспособность на тестовом контроллере
