...
### Тесты программы находятся в tests.cpp
...
Покрытие нашего кода тестами
Для оценки покрытия кода был использован инструмент OpenCppCoverage. Запуск осуществлялся с помощью команды:
...
Вывод: В ходе выполнения работы были написаны модульные тесты для функций linearModel и nonlinearModel. Все тесты прошли успешно, что подтверждает корректность реализованных алгоритмов. Покрытие кода показало, что пользовательские функции протестированы полностью.Итоги анализа покрытия тестами:

Общий процент покрытия (включая сторонние библиотеки): примерно 26%
Покрытие пользовательского файла func.cpp: 100%

Заключение: В рамках лабораторной работы были реализованы модульные тесты для функций linearModel и nonlinearModel. Все тесты успешно завершились, что подтверждает корректную работу реализованных функций. Анализ покрытия показал, что пользовательские функции протестированы полностью.
